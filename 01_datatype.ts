...
const person: object = { name: "Son", age: 10 };
const numbers: number[] = [1, 2, 3, 4, 5, 6];
const bts: string[] = ["진", "슈가", "제이홉"];
const item: [string, number, boolean] = ["Son", 30, true];
let name: any = "Son";
...
function sayHello2(name: number): string {
  return `Hello ${name}`;
}
...
console.log(result4);
const result5 = greeting1("Hun", (name: string) => `Hello ${name}`);
console.log(result5);
...
function sayHello6(name: string) {
  // void
  console.log(`Hello ${name}`);
}
...
function getAge1(age: number | string): number | string {
  return age;
}
